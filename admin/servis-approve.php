...
$id_teknisi_lama = $_GET["id_teknisi"];
...
$result_teknisi = mysqli_query($conn, "SELECT T.id_teknisi
                                                     ...
                                                     AND T.id_teknisi != '$id_teknisi_lama'
...
if ($id_teknisi_baru == 0) {
    ...
    $confirm = mysqli_query($conn, "UPDATE tbl_servis SET status = '2', alasan = '' WHERE id_servis = '$id_servis'");
} else {
    // Update teknisi jika id_teknisi_baru tidak 0
    $confirm = mysqli_query($conn, "UPDATE tbl_servis SET id_teknisi = '$id_teknisi_baru', status = '3', alasan = '' WHERE id_servis = '$id_servis'");
}
if ($confirm) {
    header("Location: servis.php");
} else {
    header("Location: servis-approve.php?id=$id_servis");
}
